...
export const addClassToImages: Plugin = (): Transformer => {
          ...
          if (node.type === 'text') {
            ...
            if (matches.length > 0) {
              matches.forEach(match => {
                ...
                const link = match[2] ? `${baseLink}#${slugify(match[1], { lower: true })}` : baseLink;
 ...
 export const getToc = (tree: Root): TocItem[] => {
    return tree.children
      .filter(
        (node): node is Element =>
          ...
          (node.tagName === "h1" ||
            node.tagName === "h2" ||
            node.tagName === "h3") && node.properties.className !== 'no-toc'
      )
      .map((node) => ({
    ...
    visit(tree, 'element', (node: Element) => {
      if (node.tagName === 'p') {
        node.children.forEach(child => {
          if (child.type === 'element' && typeof child.properties?.class === 'string') {
  ...
  export const getObsidianGraphData = (slug: string, title: string, tree: Root): ObsidianLinks => {
    ...
    const links: LinkObject[] = obsidianLinks.map(link => ({
      source: slug,
